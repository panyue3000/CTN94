...
PROC FREQ DATA=DSM_51_0;
RUN;

%macro DSM_A(DSM_I, DSM_O);
	 IF &DSM_I = 4 THEN &DSM_O=0;
ELSE IF &DSM_I IN (1 2 3) THEN &DSM_O=1;
%mend;
...
%DSM_A( DSOPISCO, DSMOPI);
%DSM_A( DSALCSCO, DSMAL);
%DSM_A( DSAMPSCO, DSMAM);
%DSM_A( DSTHCSCO, DSMCA);
%DSM_A( DSCOCSCO, DSMCO);
%DSM_A( DSSEDSCO, DSMSE);
