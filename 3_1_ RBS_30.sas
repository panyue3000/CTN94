...
/*IN CTN30 THERE IS NO 96 na OPTION*/
%macro VAR(A);
IF &A IN (-2 -1) THEN &A=.;
%mend;
...
PROC SQL;
   CREATE TABLE RBS30_01 AS 
   SELECT t1.PATIENTNUMBER AS WHO, 
          t1.VISIT, 
/*		  t1.RBS0C1,*/
          t1.RBS0C1 AS T_P, /*TOTAL # OR PARTNER*/
          t1.RBS0C2 AS T_FP, 
          t1.RBS0C3 AS T_MP, 
          t1.RBS0C4 AS T_ISMALE
       FROM CTN30.T_FRRBS t1;
QUIT;

PROC FREQ DATA=RBS30_01 ;
TABLE
T_P*(T_FP T_MP) 
T_P
T_FP
...
PROC SQL;
   ...
   SELECT t1.PATIENTNUMBER AS WHO, 
          ...
          t1.RBS0E1C AS MVW_CDM, /*MEN HAVE VAGINAL SEX WITH WOMEN CONDOM USE*/
      ...
      FROM CTN30.T_FRRBSM t1;
QUIT;

PROC FREQ DATA=RBS30M_01;
RUN;
...
PROC SQL;
   ...
   SELECT t1.PATIENTNUMBER AS WHO, 
      ...
      FROM CTN30.T_FRRBSF t1;
QUIT;

PROC FREQ DATA=RBS30W_01;
RUN;
...
PROC SQL;
   ...
   SELECT t1.PATIENTNUMBER AS WHO, 
      ...
      FROM CTN30.T_FRRBSN t1;
QUIT;

PROC FREQ DATA=RBS30A_01;
RUN;
...
/*CHECK DUPLICATE*/
PROC SQL;
   CREATE TABLE WORK.FILTER_FOR_RBS30 AS 
   SELECT DISTINCT 
	      t1.WHO, 
          t1.VISIT,
		  COUNT(T1.WHO) AS COUNT
      FROM WORK.RBS30_1 t1
	  GROUP BY T1.WHO, VISIT
	  ORDER BY COUNT DESC
